...
def main():
    ...
    maps = ["rcc/maps/level1.txt", "rcc/maps/level2.txt",
            "rcc/maps/level3.txt", "rcc/maps/level4.txt"]
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    enemy_bullets = []
                    for enemy in enemys:
                        enemies.append(FlyingEnemy(enemy[0],
                                                   enemy[1], "FlyingEnemy"))
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and shoot_cooldown <= 0:
                    ...
                    rel_x, rel_y = mp[0] - player.player_rect.x-scroll[0],\
        ...
        for bullet in bullets:
            ...
            for enemy in enemies:
                try:
                    if pygame.Rect(bullet.x-scroll[0]-25,
                            ...
                            .colliderect(pygame.Rect(enemy.x-scroll[0] + 
                                                     enemy.rect.width,
                                                     enemy.y-scroll[1] + 
                                                     enemy.rect.height, 
                                                     enemy.rect.width,
                        ...
                        screen_shake = 10
                        for x in range(50):
                            particles.append([bullet.x+random.
                                             randrange(-20, 20),
                                              bullet.y+random.randrange
                                              (-10, 10), random.randrange
                                              (-3, 3), 10, 3, random.choice
                                              ([(221, 66, 70), 
                                                (223, 214, 138)])])
            ...
            for tile in tile_rects:
                if pygame.Rect(tile[0]-scroll[0],
                               tile[1]-scroll[1], tile[2],
                               tile[3]).colliderect\
                            (pygame.Rect(bullet.x-scroll[0],
                                         bullet.y-scroll[1], 4, 4)):
                    if pygame.Rect(bullet.x-scroll[0]-25, 
                                   bullet.y-scroll[1]-25, 70, 70)\
                            .colliderect(pygame.Rect(player.player_rect.x - 
                                                     scroll[0],
                                                     player.player_rect.y 
                                                     -scroll[1],
                    ...
                    for enemy in enemies:
                        if pygame.Rect(bullet.x-scroll[0]-25,
                                       bullet.y-scroll[1]-25, 50, 50).colliderect(pygame.Rect(enemy.x-scroll[0]+enemy.rect.width, enemy.y-scroll[1]+enemy.rect.height, enemy.rect.width, enemy.rect.height)):
